...
<xsl:key name="allStandardStyles" match="/node()/simple_instance[type='Element_Style']" use="name"/> 
...
<xsl:variable name="techSlots" select="/node()/slot[name = $thisClass/template_slot]" />
...
<xsl:variable name="thisFamilies" select="key('allTechFamily', current()/name)"/>
	...
	{   
			...
			"member_of_technology_product_families":[<xsl:for-each select="$thisFamilies">{
				"id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>",
				<xsl:variable name="combinedMap" as="map(*)" select="map{
				'name': string(translate(translate(current()/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')'))
				}"/>
			...
			"vendor_lifecycle":[<xsl:for-each select="$thislifecycleModelStages"><xsl:variable name="thisLifecycleStatus" select="$allLifecycleStatii[name = current()/own_slot_value[slot_reference = 'lcm_lifecycle_status']/value]"/>{"id":"<xsl:value-of select="current()/name"/>", "start_date":"<xsl:value-of select="current()/own_slot_value[slot_reference='lcm_status_start_date_iso_8601']/value"/>",<xsl:variable name="combinedMap" as="map(*)" select="map{
				'status': string(translate(translate($thisLifecycleStatus/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')')) 
				}"/>
			<xsl:variable name="resultCombined" select="serialize($combinedMap, map{'method':'json', 'indent':true()})" />
			<xsl:value-of select="substring-before(substring-after($resultCombined,'{'),'}')" />,"statusId":"<xsl:value-of select="eas:getSafeJSString($thisLifecycleStatus/name)"/>","order":"<xsl:value-of select="$thisLifecycleStatus/own_slot_value[slot_reference='enumeration_sequence_number']/value"/>"}<xsl:if test="not(position()=last())">,</xsl:if></xsl:for-each>],
			"caps":[<xsl:for-each select="$thisTechCap">{<xsl:variable name="combinedMap" as="map(*)" select="map{
				'name': string(translate(translate(current()/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')')),
				'description': string(translate(translate(current()/own_slot_value[slot_reference = ('description')]/value,'}',')'),'{',')'))
				}"/>
			<xsl:variable name="resultCombined" select="serialize($combinedMap, map{'method':'json', 'indent':true()})" />
			...
			<xsl:for-each select="$thisTPR[own_slot_value[slot_reference = 'implementing_technology_component']/value]">
				...
				<xsl:variable name="thisStandardStyles" select="key('allStandardStyles', $thisStandardStrengths/own_slot_value[slot_reference = 'element_styling_classes']/value)"/>
			<!--	<xsl:variable name="thisTechComp" select="$allTechComponents[name=current()/own_slot_value[slot_reference='implementing_technology_component']/value]"/> -->
				...
				"allStandards":[<xsl:for-each select="$thisTechProdStandards">
					<xsl:variable name="thisStandardStrengths" select="$allStandardStrengths[name = current()/own_slot_value[slot_reference = 'sm_standard_strength']/value]"/>
					<xsl:variable name="thisStandardStyle" select="key('allStandardStyles', $thisStandardStrengths/own_slot_value[slot_reference = 'element_styling_classes']/value)"/>
					<xsl:variable name="thisGeo" select="key('geo', current()/own_slot_value[slot_reference = 'sm_geographic_scope']/value)"/>
					<xsl:variable name="thisOrg" select="key('actor', current()/own_slot_value[slot_reference = 'sm_organisational_scope']/value)"/>

					{
						<xsl:variable name="combinedMap" as="map(*)" select="map{
						'id': string(translate(translate(current()/name,'}',')'),'{',')')),
						'name': string(translate(translate(current()/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')')),
						'stdStrength': string(translate(translate($thisStandardStrengths/own_slot_value[slot_reference = ('enumeration_value')]/value,'}',')'),'{',')')),
						'stdStyle': string(translate(translate($thisStandardStyle/own_slot_value[slot_reference = ('element_style_class')]/value,'}',')'),'{',')')),
						'stdColour': string(translate(translate($thisStandardStyle/own_slot_value[slot_reference = ('element_style_colour')]/value,'}',')'),'{',')'))
					}" />
					<xsl:variable name="resultCombined" select="serialize($combinedMap, map{'method':'json', 'indent':true()})" />
					<xsl:value-of select="substring-before(substring-after($resultCombined,'{'),'}')"/>,
					"orgScope":[<xsl:for-each select="$thisOrg">
						{
							<xsl:variable name="combinedMap" as="map(*)" select="map{
								'id': string(translate(translate(current()/name,'}',')'),'{',')')),
								'type': string(translate(translate(current()/type,'}',')'),'{',')')),
								'name': string(translate(translate(current()/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')'))
							}" />
							<xsl:variable name="resultCombined" select="serialize($combinedMap, map{'method':'json', 'indent':true()})" />
							<xsl:value-of select="substring-before(substring-after($resultCombined,'{'),'}')"/>
					...
					"geoScope":[<xsl:for-each select="$thisGeo">
						{
							<xsl:variable name="combinedMap" as="map(*)" select="map{
								'id': string(translate(translate(current()/name,'}',')'),'{',')')),
								'type': string(translate(translate(current()/type,'}',')'),'{',')')),
				...
				</xsl:for-each>]}<xsl:if test="not(position()=last())">, </xsl:if></xsl:for-each><xsl:if test="not(position()=last())">, </xsl:if></xsl:for-each>], 
			<xsl:variable name="combinedMap" as="map(*)" select="map{
					'supplier': string(translate(translate($theSupplier/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')')),
					'lifecycleStatus': string(translate(translate($theLifecycleStatus/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')'))
				}" />
				<xsl:variable name="resultCombined" select="serialize($combinedMap, map{'method':'json', 'indent':true()})" />
				<xsl:value-of select="substring-before(substring-after($resultCombined,'{'),'}')"/>,

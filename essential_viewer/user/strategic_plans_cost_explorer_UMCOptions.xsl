        ...
        <html>
            <head>
                ...
                <style>
                    ...
                    /* Options table */
                    .options-panel{background:#FFFFFF;border:1px solid #E5E7EB;border-radius:8px;padding:16px 20px;margin-bottom:24px}
                    .options-title{font-weight:700;color:#361A54;font-size:1.45rem;margin-bottom:12px}
                    .options-table{width:100%;border-collapse:collapse;font-size:1.2rem}
                    .options-table th{background:#361A54;color:#fff;text-align:left;padding:12px 14px;font-weight:600;font-size:1.15rem}
                    .options-table td{padding:12px 14px;border-bottom:1px solid #E5E7EB;vertical-align:top}
                    .options-table tr:hover td{background:#FBF9FF}
                    .options-table input[type=checkbox]{width:22px;height:22px;cursor:pointer;accent-color:#993AFF}
                    .options-table td.impact-cell{font-size:1.1rem;color:#4B5563;line-height:1.45}
                </style>
                <script type="text/javascript">
                    ...
                    var OPTIONS = [
                        ...
                        {id:3, initiatives:['S094','S097','S119'], summary:'ERP - Finance first; HR follows', option:'Reduce costs by 20 million equally over all years for S094, S097, S119',
                         impact:'Finance first, HR later: Replace Finance and redesign the Chart of Accounts first, while strengthening establishment and master data; defer HR/HCM to the next planning cycle. Delivers targeted improvements where the underlying issue is related to UCL process and policy. Does not address any issues that would be improved through improved data sharing and system integration, such as the integrated transformation needed to fully support research excellence, particularly across research administration, financial insight and funder requirements.'},
                        {id:4, initiatives:['S094','S097','S119'], summary:'ERP - Improve policy &amp; process; retain ERP', option:'Reduce costs by 30 million equally over all years for S094, S097, S119',
                         impact:'Delivers targeted improvements where the underlying issue is related to UCL process and policy. Does not address any issues that would be improved through improved data sharing and system integration, such as the integrated transformation needed to fully support research excellence, particularly across research administration, financial insight and funder requirements. Reduces near-term delivery overload and disruption but accepts SR6 legacy-system risk for longer. Core constraints, including the Chart of Accounts remain weakening financial resilience.'},
                        {id:5, initiatives:['E014'], summary:'Admissions - Replace platform and some process', option:'Reduce all costs for E014 by 50%',
                         impact:'Saving and improvement from changes to Admission processes would be more gradual and would not have a strategic initiative to support it within the DSP current plan timescales. SR2 is a strategic risk for UCL. The experience part (S2.2) is in part supported by this initiative and it is likely that only superficial fixes to the experience could be made without the investment in changes to Admissions processes. The interface might look better for instance but the length of time for an application to be processed may stay the same.'},
                        {id:6, initiatives:['R008','S088','S113','S115','S124'], summary:'Remove all funding for 1 research initiative and 4 supporting initiatives (Priority 4 - 7), retain all Education', option:'Retain only P0 - P3 and force keep all Education (removes the P4 - P7 initiatives below)',
                         impact:'Removal of the following: R008, Innovation environments to drive discoveries and keep UCL research ethical and safe; S088, Inside UCL for key estates and facilities journeys; S113, Digital Foundations for Governance, Risk and Compliance; S115, CRM for Strategic Engagement; S124, Simplify Priority Services - Workplace Health.'},
                        {id:7, initiatives:['T077','T078','T079'], summary:'AI - Make UCL ready for academic and research ambition', option:'Double costs for T077, T078, T079',
                         impact:'Reacting to the evolving clarity that AI is a foundational part of the incoming Academic Strategy and high on Education and Research agendas this change is to fund the ability for UCL to establish AI sooner and in a more ambitious way. This will enable UCL to be AI ready to support the ambition of AI in Education, Research and Supporting People and across our core systems.'}
                    ];
                    ...
                    // True if a plan's name/ea_reference contains any of the given refs
                    function planHasAnyRef(planId, refs) {
                        for (var i = 0; i &lt; refs.length; i++) { if (planHasRef(planId, refs[i])) return true; }
                        return false;
                    }
                    function cloneCost(c, amount) {
                        return {amount: amount, name: c.name, recurrence: c.recurrence, startDate: c.startDate, endDate: c.endDate};
                    }
                    // Combined raw total for a set of refs (used to scale a fixed £M reduction across the group)
                    function groupRawTotal(refs) {
                        var total = 0;
                        planCosts.forEach(function(pc) {
                            if (planHasAnyRef(pc.id, refs)) { pc.costs.forEach(function(c){ total += c.amount; }); }
                        });
                        return total;
                    }
                    ...
                    function getPlanCosts(planId) {
                        ...
                        // ===== Option 1: R005 - reduce Research Computing Platform by 25% =====
                        if (activeOptions[1] &amp;&amp; planHasRef(planId, 'r005')) {
                            costs = costs.map(function(c){ return cloneCost(c, c.amount * 0.75); });
                        }

                        // ===== Option 2: S094/S097/S119 - accelerate: spread FY4+FY5 costs over FY1-3 =====
                        // FY indexing (Aug-Jul): FY1=2025, FY2=2026, FY3=2027, FY4=2028, FY5=2029.
                        if (activeOptions[2] &amp;&amp; planHasAnyRef(planId, ['s094','s097','s119'])) {
                            var y4y5 = 0;
                            costs.forEach(function(c){ var fy = fyStartYear(c.startDate); if (fy === 2028 || fy === 2029) y4y5 += c.amount; });
                            var addPerEarly = y4y5 / 3;
                            costs = costs.map(function(c){
                                var fy = fyStartYear(c.startDate);
                                if (fy === 2028 || fy === 2029) return cloneCost(c, 0);
                                if (fy === 2025 || fy === 2026 || fy === 2027) return cloneCost(c, c.amount + addPerEarly);
                                return cloneCost(c, c.amount);
                            });
                        }

                        // ===== Option 3: ERP Finance first - reduce S118 costs by 30% =====
                        if (activeOptions[3] &amp;&amp; planHasRef(planId, 's118')) {
                            costs = costs.map(function(c){ return cloneCost(c, c.amount * 0.7); });
                        }

                        // ===== Option 4: ERP Improve policy - reduce S118 costs by 50% =====
                        if (activeOptions[4] &amp;&amp; planHasRef(planId, 's118')) {
                            costs = costs.map(function(c){ return cloneCost(c, c.amount * 0.5); });
                        }

                        // ===== Option 5: E014 - reduce all costs by 50% =====
                        if (activeOptions[5] &amp;&amp; planHasRef(planId, 'e014')) {
                            costs = costs.map(function(c){ return cloneCost(c, c.amount * 0.5); });
                        }

                        // Option 6 is a priority-filter preset (P0-P3 + keep all Education), applied in filterPlans - no cost change here.

                        // ===== Option 7: T077/T078/T079 - double costs (AI ambition) =====
                        if (activeOptions[7] &amp;&amp; planHasAnyRef(planId, ['t077','t078','t079'])) {
                            costs = costs.map(function(c){ return cloneCost(c, c.amount * 2); });
                        }
                    ...
                    // True when the "keep all Education" override should protect this plan.
                    // Education initiatives are identified by an [Exxx] reference in the plan name.
                    // When on, keep every [Exxx] plan regardless of priority.
                    function educationKeepOverride(planId) {
                        if (!keepEducationP5) return false;
                        var plan = allPlans.find(function(p) { return p.id === planId; });
                        return plan &amp;&amp; plan.name &amp;&amp; /\[E\d+\]/.test(plan.name);
                    }
                    ...
                    function renderOptionsTable() {
                        ...
                        OPTIONS.forEach(function(o) {
                            html += '&lt;tr&gt;';
                            html += '&lt;td style="text-align:center;"&gt;&lt;input type="checkbox" class="opt-check" data-opt="' + o.id + '"/&gt;&lt;/td&gt;';
                            html += '&lt;td style="text-align:center;font-weight:700;color:#361A54;"&gt;' + o.id + '&lt;/td&gt;';
                            html += '&lt;td&gt;' + o.initiatives.join(', ') + '&lt;/td&gt;';
                            html += '&lt;td&gt;&lt;div style="font-weight:600;color:#361A54;"&gt;' + o.summary + '&lt;/div&gt;&lt;div style="font-size:1.05rem;color:#6B7280;margin-top:3px;"&gt;' + o.option + '&lt;/div&gt;&lt;/td&gt;';
                            html += '&lt;td class="impact-cell"&gt;' + o.impact + '&lt;/td&gt;';
                            html += '&lt;/tr&gt;';
                        });
                        tbody.innerHTML = html;
                        var checks = document.querySelectorAll('.opt-check');
                        for (var i = 0; i &lt; checks.length; i++) {
                            checks[i].addEventListener('change', function() {
                                var id = parseInt(this.getAttribute('data-opt'), 10);
                                if (this.checked) { activeOptions[id] = true; } else { delete activeOptions[id]; }
                                // Options 2, 3 and 4 are alternative ERP approaches - mutually exclusive
                                var erpGroup = [2, 3, 4];
                                if (this.checked &amp;&amp; erpGroup.indexOf(id) !== -1) {
                                    erpGroup.forEach(function(other){
                                        if (other !== id) {
                                            delete activeOptions[other];
                                            var oc = document.querySelector('.opt-check[data-opt="' + other + '"]');
                                            if (oc) oc.checked = false;
                                        }
                                    });
                                }
                                // Option 6 is a preset: retain only P0-P3 + force keep all Education
                                if (id === 6) {
                                    topPriorityOnly = this.checked;
                                    keepEducationP5 = this.checked;
                                    var tp = document.getElementById('filterTopPriority');
                                    var ke = document.getElementById('filterKeepEducation');
                                    if (tp) tp.checked = this.checked;
                                    if (ke) ke.checked = this.checked;
                                }
                                renderView();
                            });
                        }
            ...
            <body>
                ...
                <div class="view-wrapper">
                    <h1 style="margin-bottom:5px;">Strategic Plans Cost Explorer &#8212; UMC Options</h1>
                    <p style="color:#6B7280;margin-bottom:20px;">Select one or more options below to model their impact on the strategic plan costs. The impact on ambition for each option is shown in the table.</p>
                    <div class="options-panel" id="optionsPanel">
                        <div class="options-title">Options under consideration</div>
                        <table class="options-table">
                            <thead>
                                <tr>
                                    <th style="width:70px;">Apply</th>
                                    <th style="width:70px;">Option</th>
                                    <th style="width:120px;">Affected initiative</th>
                                    <th style="width:26%;">Summary of change</th>
                                    <th>Impact on ambition</th>
                                </tr>
                            </thead>
                    ...
                    <div class="filters-bar filters-bar-reductions">
                        ...
                        <div class="toggle-filter"><label for="filterKeepEducation"><input type="checkbox" id="filterKeepEducation"/><xsl:text> </xsl:text>Force keep all Education</label></div>
                    </div>
                    ...
                    <div class="filters-bar filters-bar-reductions">
                        <span class="filters-group-label">Funding:</span>

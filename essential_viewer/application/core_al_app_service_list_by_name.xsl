...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
		...
		<xsl:choose>
			<xsl:when test="string-length($viewScopeTermIds) > 0">
				...
				<xsl:variable name="pageLabel">
					<xsl:value-of select="eas:i18n('Application Service Catalogue by Name')"/>
				</xsl:variable>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="pageLabel" select="$pageLabel"/>
					<xsl:with-param name="inScopeAppSvcs" select="$allAppServices[own_slot_value[slot_reference = 'element_classified_by']/value = $viewScopeTerms/name]"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="inScopeAppSvcs" select="$allAppServices"/>
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
				...
				<div class="container-fluid">
								...
								<div class="altViewName">
									...
									<span class="text-darkgrey">
										<xsl:call-template name="RenderCatalogueLink">
											<xsl:with-param name="theCatalogue" select="$appSvcListAsTableCatalogue"/>
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<p>
								<xsl:value-of select="eas:i18n('Please click on one of the Application Services below to navigate to the required view')"/>
							</p>
							<div>
								...
								<div class="AlphabetQuickJumpLinks hidden-xs">
									...
									<xsl:variable name="anInFocusInstances" select="$inScopeAppSvcs"></xsl:variable>
								...
								<xsl:call-template name="Index">
									...
									<xsl:with-param name="appServices" select="$inScopeAppSvcs"/>
								</xsl:call-template>
	...
	<xsl:template name="Index">
		...
		<div class="catalogItems">
			<ul>
				<xsl:apply-templates select="$appServices[((starts-with(own_slot_value[slot_reference = 'name']/value, $letter)) or (starts-with(own_slot_value[slot_reference = 'name']/value, $letterLow)))]" mode="ApplicationService">
	...
	<xsl:template match="node()" mode="ApplicationService">
		<li>
	...
	<xsl:template name="eas:renderIndex">
		...
		<xsl:for-each select="$anIndexKeys">
			<xsl:call-template name="Index">
				...
				<xsl:with-param name="appServices" select="$theInFocusInstances"/>
			</xsl:call-template>
